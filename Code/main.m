...
downsampled_data = downsample(data(30:end,:),donwsampling_rate);           % downsampling the data
...
repeated_labels=repmat(1:5,1,ceil(size(downsampled_data,1)/5));            % generaqting labels
...
figure(); plot(donwsampled_and_labeled_data(:,1:10))                        % plotting sensor readings
legend(...
    'S_1','S_2','S_3','S_4','S_5','S_6','S_7','S_8','S_9','S_10', 'location', 'southeast')  % setting the plot legend
...
mdl=fitcknn(train_data(1:10,:)',train_data(11,:)','NumNeighbors',K);         % fitting the KNN model
KNNPred=predict(mdl,test_data(1:10,:)');                                    % running the KNN model on the test data
